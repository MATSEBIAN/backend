...
DATABASE_URL = os.environ.get('DATABASE_URL', '').replace('postgresql+pg8000://', 'postgresql://').replace('postgres://', 'postgresql://')
...
def cors(r):
    ...
    r.headers['Access-Control-Allow-Headers'] = 'Content-Type, Authorization, X-Empresa-ID'
...
def upload_transaction():
    ...
    msg = client.messages.create(
        ...
        messages=[{"role":"user","content":[
            content_block,
            {"type":"text","text":"Analiza esta factura y extrae información en JSON. CRÍTICO: año 2026 salvo que la factura diga otro. Detecta si es ABONO (palabras: ABONO, CARGO RECTIFICATIVO, NOTA DE CRÉDITO, totales negativos). CIF del PROVEEDOR: busca en cabecera, pie, registro mercantil, limpia ES- y guiones. LOCAL: CRÍTICO - ciudad donde se ENTREGÓ el producto/servicio. Busca PRIMERO en ENVIADO A / SHIP TO / ENTREGADO A (ej: Conway enviado a Dos Hermanas = Dos Hermanas). Si no hay dirección de envío, usa la ciudad de la sucursal del proveedor (ej: OBRAMAT JEREZ = Jerez). NUNCA usar la ciudad de facturación del cliente. Solo el nombre de la ciudad, sin provincia ni CP. ACREEDOR: nombre fiscal de quien recibe (cliente). Responde SOLO con JSON sin markdown: {importe_total: número positivo, iva: número positivo o 0, irpf: número positivo o 0, proveedor: texto, cif_proveedor: texto limpio sin ES- ni guiones, num_factura: texto, fecha: YYYY-MM-DD, local: solo ciudad, acreedor: nombre fiscal cliente, esAbono: bool, categoria: una de alimentacion/bebidas/suministros/nominas/alquiler/marketing/otros}"}
        ]}]
    )
    raw = msg.content[0].text.strip()
    raw = re.sub(r'^```json|^```|```$', '', raw, flags=re.MULTILINE).strip()
